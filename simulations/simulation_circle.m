...
% temporal straightening paper (2019). Simulates circula trajectories by
% keeping the length and curvatures/angles of the vectors constant.
close all; clear all; clc;

%% global constants
ExpParam.numFrames  = 3;
ExpParam.avgLocCurv = 30;
...
lengths    = zeros(ExpParam.numFrames) + (max_length - min_length) * 0.5 + min_length;
...
ExpParam.thetas       = abs(deg2rad(zeros(ExpParam.numFrames, 1) + ExpParam.avgLocCurv));
...
cw_ccw_order = ones(ExpParam.numFrames);
