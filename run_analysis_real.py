...
def set_params():
    ...
    --------
        ...
        sequences: Array containing sequencess, e.g., {'groundtruth'}
        subjects: Array containing the names of the subjects.
        conditions: Array containing the names of the conditions.
    """
    ...
    params['seed'] = 0 # TODO: figure out whether to keep it or not

    # Set processing parameters for perceptual data.
    params['process'] = {
    ...
    if params['process']['experimenter'] == 'yoon': 
    ...
    elif params['process']['experimenter'] == 'corey':
        sequences  = ['groundtruth']
        subjects   = ['cmz', 'CMZ']
        conditions = [
            'pilot_movie1', 
        ...
        raise NameError("params['process']['experimenter'] must be 'yoon' or 'corey'")
...
def run_analysis(params, metadata, sequences, subjects, conditions):
    ...
    -------
        params: Dictionary containing parameters for the analysis.
        metadata: Dictionary containing hardcoded metadata for the analysis, only used if seed > 0. If seed - 0, metadata = None.
        sequences: Array containing sequences, e.g., {'groundtruth'}
        subjects: Array containing the names of the subjects.
        conditions: Array containing the names of the conditions.
    """

    # Create save directory iff it does not already exist. 
    save_dir = f'results/{params['process']['experimenter']}/{date.today()}'
    ...
    for subj in subjects:
        for seq in sequences:
            for cond in conditions:
                ...
                data = ExperimentalData(experiment, params['process'])
                data.load_raw_data()
                data.process()

                # Plot as sanity check
                ax = sns.heatmap(data = data.p_correct, cmap='Blues')
                ax.invert_yaxis()
                plt.show()

                # TODO: Maybe do it later
                # Create synthetic null data.
                # null_data = ExperimentalDataNull(
                #     experiment, 
                #     params['process'], 
                #     params['curvature'],
                #     # params['null']
